...
//   onUpdateAreaRectangleCancel } : PdfViewerProps) => {
...
//     const [justCreatedRectangle, setJustCreatedRectangle] = useState<boolean>(false)
//     const [resizeHandleType, setResizeHandleType] = useState<'bottom-right' | 'top-left' | 'center' | null>(null)
...
//         // Используем абсолютные координаты для всей документа
//         const absoluteY = y

//         setStartX(x)
//         setStartY(y)
//         setStartPoint({ x, y: absoluteY, page: pageNumber })
//         setIsResizing(false)
//         setTempRect({
//           xMin: x / scale,
//           xMax: x / scale,
//           yMin: absoluteY / scale,
//           yMax: absoluteY / scale,
//           minPage: pageNumber,
//           maxPage: pageNumber
//         })
//       }
...
//         })
//       } else if (mode.type === 'CREATE_RECTANGLE' && tempRect && onCreateRectangle && startPoint) {
//         const normalizedRect = {
//           xMin: Math.min(tempRect.xMin, tempRect.xMax),
//           xMax: Math.max(tempRect.xMin, tempRect.xMax),
//           yMin: Math.min(tempRect.yMin, tempRect.yMax),
...
//           })
//         }
//       } else if (mode.type === 'CREATE_RECTANGLE' && startX !== null && startY !== null && startPoint) {
//         const containerRect = containerRef.current?.getBoundingClientRect()
//         if (!containerRect) return
...
//         console.log('Mouse move - current page:', pageNumber,
//           'start page:', startPoint.page,
//           'minPage:', minPage,
//           'maxPage:', maxPage)
...
//     const handleResizeMouseDown = (e: React.MouseEvent, area: PdfArea, handleType: 'bottom-right' | 'top-left' | 'center') => {
//       e.preventDefault()
//       e.stopPropagation()
...
//       if (!clickableAreas || mode.type === 'CREATE_RECTANGLE' || isResizing) return
...
//         // Находим страницу, на которой находится клик
...
//         // Рассчитываем локальные координаты для найденной страницы
//         let localY = y * scale
...
//         return clickedPage >= area.rectangle.minPage &&
...
//             e.preventDefault()
//           }
//           else if (mode.type === 'UPDATE_AREA_RECTANGLE' && onUpdateAreaRectangleCancel) {
...
//       }
//     }, [mode.type, startX, tempRect, onCreateRectangleCancel, onUpdateAreaRectangleCancel])
...
//     const onPageLoadSuccess = (pageNumber: number) => (page: { originalHeight: number }) => {
//       if (!containerRef) return
//       const canvas = containerRef.current?.querySelector(`.react-pdf__Page[data-page-number="${pageNumber}"] .react-pdf__Page__canvas`) as HTMLCanvasElement
//       if (canvas) {
...
//       const pages = pageContainerRef.current.querySelectorAll('.react-pdf__Page__canvas')
//       const observer = new ResizeObserver(entries => {
//         const heights = Array.from(entries).map(entry => entry.contentRect.height)
...
//             const pageAreas: PdfArea[] = areas.filter(area =>
//               pageNumber >= area.rectangle.minPage && pageNumber <= area.rectangle.maxPage
//             )
...
//                 />
//                 {clickableAreas && mode.type !== 'CREATE_RECTANGLE' && !isResizing && (
//                   <div className="document-click-layer"/>
//                 )}
//                 <div className="visual-layer">
//                   {pageAreas.map((area) => {
//                     const currentRect = tempRect && resizingArea?.id === area.id ? tempRect : area.rectangle
...
//                     }
//                     else if (pageNumber === currentRect.maxPage && currentRect.minPage !== currentRect.maxPage) {
//                       relativeYMax = relativeYMax - pageHeights[pageNumber - 1]
//                     }
...
//                         />
//                         {mode.type === 'UPDATE_AREA_RECTANGLE' && mode.areaId === area.id && (
//                           <>
//                             <ResizeHandler
...
//                               }}
//                               scale={scale}
//                               onMouseDown={(e) => handleResizeMouseDown(e, area, 'bottom-right')}
//                             />
//                             <ResizeHandler
//                               className='top-left'
//                               coordinates={{
...
//                               }}
//                               scale={scale}
//                               onMouseDown={(e) => handleResizeMouseDown(e, area, 'top-left')}
//                             />
//                             <ResizeHandler
//                               className='center'
//                               coordinates={{
...
//                               }}
//                               scale={scale}
//                               onMouseDown={(e) => handleResizeMouseDown(e, area, 'center')}
//                             />
//                           </>
//                         )}
//                         <AreaName
...
//                     )
//                   })}
//                   {mode.type === 'CREATE_RECTANGLE' && tempRect && (
//                     Array.from({ length: tempRect.maxPage - tempRect.minPage + 1 }, (_, i) => {
...
//                       // Правильно рассчитываем координаты для каждой страницы
//                       let pageYMin = 0
